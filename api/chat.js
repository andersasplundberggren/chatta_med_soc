...
export default async function handler(req, res) {
    ...
    try {
        // UPPDATERAT - Ta emot conversationHistory
...
`;
        ...
        // NYTT - Bygg messages-array med historik
        const messages = [
            { role: 'system', content: context }
        ];

        // Lägg till konversationshistoriken (max 10 senaste meddelanden för att hålla nere kostnad)
        const recentHistory = conversationHistory.slice(-10);
        messages.push(...recentHistory);

        // Om det aktuella meddelandet inte redan finns i historiken, lägg till det
        if (recentHistory.length === 0 || recentHistory[recentHistory.length - 1].content !== message) {
            messages.push({ role: 'user', content: message });
        }

        console.log(`Skickar ${messages.length} meddelanden (inkl. ${recentHistory.length} historiska)`);

        // Generera svar med historik
